...
// Exported so db/init.ts can seed a matching `users` row for every trader that appears in
// seeded trades.
...
export function generateSeedTrades(count: number): NewTrade[] {
  return Array.from({ length: count }, () => {
    ...
    return {
      ...
      quantity: (Math.floor(Math.random() * 100) + 1) * 100, // 100-10,000, in lots of 100 — always a whole number, matching the `quantity INT` column
